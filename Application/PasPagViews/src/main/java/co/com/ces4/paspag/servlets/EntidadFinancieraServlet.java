 ...
 * @author cristian
 */
@WebServlet(name = "EntidadFinancieraServlet", urlPatterns = {"/EntidadFinancieraServlet"})
public class EntidadFinancieraServlet extends HttpServlet {
    ...
    protected void limpiarRedireccionar(HttpServletRequest request, HttpServletResponse response)
        ...
        response.setContentType("text/html;charset=UTF-8");
        try (PrintWriter out = response.getWriter()) {
            EntidadFinancieraJpaController controller = (EntidadFinancieraJpaController) getServletContext().getAttribute("entidadFinancierJpa");
            TipoDocumento tipodoc = TipoDocumento.values()[TipoDocumento.values().length - 1];
            List listaPersonas = controller.findEntidadFinancieraEntities();
            request.setAttribute("listaPersonas", listaPersonas);
            request.setAttribute("tipo_seleccionado", tipodoc.toString());
            limpiar(request, response);
            request.getRequestDispatcher("EntidadFinanciera.jsp").forward(request, response);
        }
    }
    ...
    protected void metGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            limpiar(request, response);
            EntidadFinancieraJpaController controller = (EntidadFinancieraJpaController) getServletContext().getAttribute("entidadFinancierJpa");
            if (accion.equals("listEntidadesFinancieras") || accion.equals("limpiarRedireccionar")) {
            ...
            if (accion.equals("set_data")) {
                ...
                EntidadFinanciera persona = controller.findEntidadFinanciera(primaryKey);
    ...
    protected void metPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            EntidadFinanciera entidadFinanciera = new EntidadFinanciera(ciudad_nacimiento, pais_nacimiento, fecha_nacimiento, Documento, tipo_seleccionado, nombre_persona, contacto_persona, correo_persona, direccion_persona, usuario, contrasena);
            if (request.getParameter("conditional").equals("1")){
                controller.edit(entidadFinanciera);
                limpiarRedireccionar(request, response);
            }else {                
                controller.create(entidadFinanciera);
                limpiarRedireccionar(request, response);
            }
        }
    }
